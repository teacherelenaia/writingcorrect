...
async function sendLimitWarningEmail(email, plan, used, limit) {
      const planNames = { free: 'Gratuito', basic: 'B\u00e1sico', teacher: 'Profesor', center: 'Centro' };
      ...
      try {
              await fetch('https://api.resend.com/emails', {
                        ...
                        body: JSON.stringify({
                                    ...
                                    html: `<div style="font-family:sans-serif;max-width:520px;margin:auto;padding:32px;background:#f8fafc;border-radius:12px;">
                                    <h2 style="color:#0E1E35;font-size:20px;margin-bottom:8px;">Casi has agotado tu cuota mensual</h2>
                                    <p style="color:#475569;font-size:15px;">Llevas <strong>${used} de ${limit} correcciones</strong> usadas este mes con el plan <strong>${planNames[plan] || plan}</strong>. Solo te quedan <strong>${remaining}</strong>.</p>
                                    <p style="color:#475569;font-size:15px;margin-top:12px;">Para no quedarte sin correcciones a mitad de mes, puedes actualizar tu plan ahora:</p>
                                    <a href="${upgradeUrl}" style="display:inline-block;margin-top:20px;padding:12px 28px;background:#0E1E35;color:#fff;border-radius:8px;text-decoration:none;font-weight:600;font-size:15px;">Ver planes \u2192</a>
                                    <p style="color:#94a3b8;font-size:12px;margin-top:28px;">Si tienes alguna duda, responde a este email.</p>
                                    </div>`,
                        }),
              });
      } catch (_e) {}
            ...
            try {
                ...
                if (userId4Limit) {
                    ...
                    if (pRes.ok) {
                        ...
                        // Send warning email at 80% of limit (only for paid plans, only on the exact crossing correction)
