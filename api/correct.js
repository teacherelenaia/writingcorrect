...
async function sendLimitWarningEmail(email, plan, used, limit) {
  const planNames = { free: 'Gratuito', basic: 'Basico', teacher: 'Profesor', center: 'Centro' };
  const remaining = limit - used;
  const upgradeUrl = 'https://writingcorrect.com/app';
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: `\u26a0\ufe0f Solo te quedan ${remaining} correcciones este mes`,
        html: `<div style="font-family:sans-serif;max-width:520px;margin:auto;padding:32px;background:#f8fafc;border-radius:12px;"><h2 style="color:#0E1E35;font-size:20px;margin-bottom:8px;">Casi has agotado tu cuota mensual</h2><p style="color:#475569;font-size:15px;">Llevas <strong>${used} de ${limit} correcciones</strong> usadas este mes con el plan <strong>${planNames[plan] || plan}</strong>. Solo te quedan <strong>${remaining}</strong>.</p><p style="color:#475569;font-size:15px;margin-top:12px;">Para no quedarte sin correcciones a mitad de mes, puedes actualizar tu plan ahora:</p><a href="${upgradeUrl}" style="display:inline-block;margin-top:20px;padding:12px 28px;background:#0E1E35;color:#fff;border-radius:8px;text-decoration:none;font-weight:600;font-size:15px;">Ver planes \u2192</a><p style="color:#94a3b8;font-size:12px;margin-top:28px;">Si tienes alguna duda, responde a este email.</p></div>`,
      }),
    });
  } catch (_e) {}
}

async function incrementGlobalCounter(sbUrl, sbSvc) {
  try {
    await fetch(sbUrl + '/rest/v1/rpc/increment_corrections_total', {
      method: 'POST',
    ...
    if (token && texto !== '__transcribe__') {
      ...
      try {
        ...
        if (uRes.ok) {
          ...
          userEmail4Warning = uData?.email || null;
        }
        if (userId4Limit) {
          ...
          if (pRes.ok) {
            ...
            // Send warning email at 80% of limit (only for paid plans)
            if (limit !== Infinity && planKey !== 'free' && userEmail4Warning) {
              const threshold = Math.floor(limit * 0.8);
              if (effectiveUsed === threshold) {
                sendLimitWarningEmail(userEmail4Warning, planKey, effectiveUsed, limit);
              }
            }
          }
        }
...
${jsonEstructura}`;
    ...
    // Increment global corrections counter (fire and forget)
    incrementGlobalCounter(SB_URL4, SB_SVC);
    // Increment user corrections_used
    if (userId4Limit) {
      fetch(SB_URL4 + '/rest/v1/profiles?id=eq.' + userId4Limit, {
        method: 'PATCH',
        headers: { 'apikey': SB_SVC, 'Authorization': 'Bearer ' + SB_SVC, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({ corrections_used: (resultado._used_count || 0) + 1 })
      }).catch(() => {});
      // Use RPC increment instead
      fetch(SB_URL4 + '/rest/v1/rpc/increment_corrections_used', {
        method: 'POST',
        headers: { 'apikey': SB_SVC, 'Authorization': 'Bearer ' + SB_SVC, 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId4Limit })
      }).catch(() => {});
    }
